"""
Geometry-only (pre-solved) model: 4 rows x 2 columns of dowels, H=300.
...
def build_geometry_only(
    L=1000, H=300, B=140, t_plate=8,
    n_rows=4, n_cols=2,
    s_dowel=120, a_edge=100, row_spacing=60,
    d_dowel=12,
    beam_esize=50,   # coarse — whole beam away from dowel zone
    plate_esize=20,  # medium — steel plate
    dowel_esize=6,   # fine — dowel cylinders (d/2 = 6 mm)
    clearance=0.5,
):
    # ── Derived geometry ──────────────────────────────────────────────────────
    zc     = B / 2
    ...
    print("=" * 62)
    print("  GEOMETRY PARAMETERS  (n_rows=4, n_cols=2)")
    print("=" * 62)
    ...
    if os.path.exists(lockfile):
        try:
            ...
            # Lock held by a lingering ANSYS process — kill it and retry
            import subprocess
            subprocess.call(
    ...
    # ── Mesh ──────────────────────────────────────────────────────────────────
    # Three zones: coarse beam, medium plate, fine dowels.
    # SMRTSIZE,8 on the beam only — coarse smart sizing that handles thin slot
    # wall features (1 mm oversized cut) without over-refining the whole beam.
    mapdl.mshape(1, "3d")
    mapdl.mshkey(0)
    ...
    # ── Save and exit ─────────────────────────────────────────────────────────
    mapdl.save("pymapdl_presolved_4r2c", "db")
    print("Saved: out/pymapdl_presolved_4r2c.db")
...
if __name__ == "__main__":
    build_geometry_only(beam_esize=50, plate_esize=20, dowel_esize=6)
